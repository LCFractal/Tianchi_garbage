...
_KERAS_BACKEND = tfkeras.backend
_KERAS_LAYERS = tfkeras.layers
_KERAS_MODELS = tfkeras.models
_KERAS_UTILS = tfkeras.utils
...
def EfficientNet(width_coefficient,
    ...
    x = layers.BatchNormalization(axis=bn_axis, name=model_name+'_stem_bn')(x)
    x = layers.Activation(activation, name=model_name+'_stem_activation')(x)
    ...
    for idx, block_args in enumerate(blocks_args):
        ...
        x = mb_conv_block(x, block_args,
                          activation=activation,
                          drop_rate=drop_rate,
                          prefix=model_name+'_block{}a_'.format(idx + 1))
        block_num += 1
        if block_args.num_repeat > 1:
            ...
            for bidx in xrange(block_args.num_repeat - 1):
                drop_rate = drop_connect_rate * float(block_num) / num_blocks_total
                block_prefix = model_name+'_block{}{}_'.format(
                    idx + 1,
    ...
    x = layers.Conv2D(round_filters(1280, width_coefficient, depth_divisor), 1,
                      ...
                      use_bias=False,
                      kernel_initializer=CONV_KERNEL_INITIALIZER,
                      name=model_name+'_top_conv')(x)
    x = layers.BatchNormalization(axis=bn_axis, name=model_name+'_top_bn')(x)
    x = layers.Activation(activation, name=model_name+'_top_activation')(x)
    if include_top:
        x = layers.GlobalAveragePooling2D(name=model_name+'_avg_pool')(x)
        if dropout_rate and dropout_rate > 0:
            x = layers.Dropout(dropout_rate, name=model_name+'_top_dropout')(x)
        x = layers.Dense(classes,
                         activation='softmax',
                         kernel_initializer=DENSE_KERNEL_INITIALIZER,
                         name=model_name+'_probs')(x)
    else:
        if pooling == 'avg':
            x = layers.GlobalAveragePooling2D(name=model_name+'_avg_pool')(x)
        elif pooling == 'max':
            x = layers.GlobalMaxPooling2D(name=model_name+'_max_pool')(x)
